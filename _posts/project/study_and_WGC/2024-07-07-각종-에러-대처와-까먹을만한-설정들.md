...
### 최신 버전 확인
[chirpy 업데이트 로그](https://github.com/cotes2020/jekyll-theme-chirpy/releases)
<br><br>
...
npm run build
``` 
위 명령어들을 차례대로 실행하면 해결된다.

```
assets/js/dist
```
{: file='.gitignore'}
다만 위와 같은 폴더가 생겨도 git이 무시하는데, .gitignore에서 위의 경로를 주석처리 해야한다. <br>
...
```
{: file='최신화 후 jekyll 실행 시 경고 문구' }
Ruby 3.5.0부터는 logger기능을 기본적으로 지원해주지 않아서 발생하는 문제다. <br>
`bundle update listen`으로 해결했다.
<br>
...
### dist 폴더 자동 생성
그냥 jobs에 때려 박는게 안되서 ChatGPT랑 Perplexity 써가면서 찾은 방식이다. <br>
내용 보면 알겠지만 os를 두번 쓰며, Ruby 시점에서 nodejs에서 생성한 파일들을 다운받아야 한다.<br>
2시간 걸려서 찾은건데 이렇게 쓸거면 dist를 직접 github에 넣는게 좋아보여서 기록으로 남기기만 한다. (테스트 안했으니 주의)
```yml
# jobs 이전까진 다 똑같다.
...
jobs:
  ...
  build-js:
    ...
    outputs:
      js-artifact-path: ${{ steps.build-js.outputs.artifact-path }}
    steps:
      - name: Checkout
  ...
  build-ruby:
    ...
    steps:
      ...
      - name: Build assets (dist) with Rake
